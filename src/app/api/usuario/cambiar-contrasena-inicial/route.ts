...
import { esTokenExpirado, hashTokenCambioContraseña } from "@/lib/password-reset";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (tokenRecord.usado || esTokenExpirado(tokenRecord.expires)) return { status: "invalid" as const };
      ...
      await tx.usuarioCambioContraseña.updateMany({
        where: { usuarioId: tokenRecord.usuario.id, usado: false },
        data: { usado: true },
      });
    ...
    if (result.status === "invalid") {
      return NextResponse.json({ error: "El enlace no es válido, ya fue utilizado o ha expirado." }, { status: 400 });
    }
    ...
    try {
      ...
      console.error("Error enviando confirmación de contraseña:", emailError);
    }
...
export async function GET(request: Request) {
  try {
    ...
    const tokenRecord = await prisma.usuarioCambioContraseña.findUnique({
      where: { token: hashTokenCambioContraseña(token) },
      select: {
        id: true,
        usado: true,
        expires: true,
    ...
    if (!tokenRecord) {
      return NextResponse.json({ error: "Token no encontrado", valid: false }, { status: 404 });
    }
    if (tokenRecord.usado) {
      return NextResponse.json({ error: "Este token ya ha sido utilizado", valid: false }, { status: 400 });
    }
    if (esTokenExpirado(tokenRecord.expires)) {
      return NextResponse.json({ error: "El token ha expirado", valid: false }, { status: 400 });
    }

    return NextResponse.json({
      valid: true,
      usuario: { nombre: tokenRecord.usuario.nombre, email: tokenRecord.usuario.email },
    });
  } catch (error) {
