...
function estadoParaResumen(estado: EstadoObligacion): "cumplido" | "vencido" | "pendiente" | "no_aplica" {
  if (estado === "cumplida") return "cumplido";
  if (estado === "con_brechas") return "vencido";
  ...
  return (
    <Card className="border border-slate-200 bg-white shadow-sm">
      <CardContent className="pt-5 pb-4 space-y-3">
        ...
        <div className="flex flex-wrap gap-2 pt-1">
          ...
          {resultado.vencidas > 0 && (
            <span className="inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-medium bg-amber-50 text-amber-700 border border-amber-100">
              <AlertTriangle className="h-3 w-3" /> {resultado.vencidas} con brecha{resultado.vencidas !== 1 ? "s" : ""}
            </span>
          )}
        ...
        <p className="text-[10px] text-slate-400">{resultado.totalObligaciones} obligaciones evaluadas</p>
      </CardContent>
    </Card>
  );
}

export default async function CumplimientoResumenPage() {
  const [payload, hallazgos] = await Promise.all([
    getObligacionesCumplimientoEmpresa(),
    getHallazgos(),
  ]);

  const tamanoEmpresa = calcularTamañoEmpresa(payload.cantidadTrabajadores);
  const obligacionesAplicables = payload.obligaciones.filter((o) => o.aplica);

  const evaluaciones = obligacionesAplicables.flatMap((o) => {
    if (payload.centros.length === 0) {
      return [
        {
          obligacionNombre: o.nombre,
          entidadId: "empresa",
          estado: estadoParaResumen(o.estadoObligacion),
          fuenteTipo: o.fuenteTipo,
          fuenteId: o.documentoEmpresaId ?? o.obligacionClave,
          fechaVencimiento: o.vencimiento,
        },
      ];
    }

    return payload.centros.map((centro) => {
      const estadoCentro = (o.estadosPorCentro[centro.id] ?? o.estadoObligacion) as EstadoObligacion;
      return {
        obligacionNombre: o.nombre,
        entidadId: centro.id,
        estado: estadoParaResumen(estadoCentro),
        fuenteTipo: o.fuenteTipo,
        fuenteId: o.documentoEmpresaId ?? o.obligacionClave,
        fechaVencimiento: o.vencimiento,
      };
  ...
  const resultadosPorEntidad: ResultadoEntidad[] = payload.centros.map((centro) => {
    const rows = evaluaciones.filter((e) => e.entidadId === centro.id);
    const totalObligaciones = rows.filter((e) => e.estado !== "no_aplica").length;
    const cumplidas = rows.filter((e) => e.estado === "cumplido").length;
    const vencidas = rows.filter((e) => e.estado === "vencido").length;
    const pendientes = rows.filter((e) => e.estado === "pendiente").length;
    const porcentajeCumplimiento = totalObligaciones > 0
      ? Math.round((cumplidas / totalObligaciones) * 100)
      : 0;
  ...
  const globalPct = obligacionesAplicables.length > 0
    ? Math.round(
        obligacionesAplicables.reduce((sum, ob) => sum + ob.cumplimientoGlobal, 0) /
          obligacionesAplicables.length,
      )
    : 0;

  const totalCumplidas = obligacionesAplicables.filter((o) => o.estadoObligacion === "cumplida").length;
  const totalVencidas = obligacionesAplicables.filter((o) => o.estadoObligacion === "con_brechas").length;
  const totalPendientes = obligacionesAplicables.filter((o) => o.estadoObligacion === "no_cumplida").length;
  const totalDocs = obligacionesAplicables.filter((o) => o.fuenteTipo !== "manual").length;
  ...
  return (
    <div className="min-h-screen bg-slate-50/80 py-10">
      <div className="mx-auto max-w-6xl space-y-8 px-4 lg:px-0">
        <StandardPageHeader
          moduleLabel="Modulo Cumplimiento"
          title="Resumen de cumplimiento"
          description="Estado consolidado del cumplimiento normativo DS44 basado en datos Prisma de obligaciones, documentos y hallazgos."
        ...
        <div className="flex items-center gap-2 rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800">
          ...
          <span>
            Calculo automatico activo - <strong>{evaluaciones.length}</strong> evaluaciones procesadas, <strong>{totalDocs}</strong> vinculadas a evidencia documental real.
          </span>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700">
          ...
          <span>
            Dotacion considerada: <strong>{payload.cantidadTrabajadores} trabajadores</strong>
          </span>
          <span className={cn("inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold", TAMANO_CFG[tamanoEmpresa].cls)}>
            Empresa {TAMANO_CFG[tamanoEmpresa].label}
          </span>
          {payload.noAplican > 0 && (
            <span className="rounded-full bg-slate-100 px-2.5 py-0.5 text-xs text-slate-500">
              {payload.noAplican} obligacion{payload.noAplican !== 1 ? "es" : ""} no aplica{payload.noAplican !== 1 ? "n" : ""}
            </span>
          )}
        </div>

        <EstructurasObligatoriasCard cantidadTrabajadores={payload.cantidadTrabajadores} />
          ...
          {[
            {
              ...
              value: `${globalPct}%`,
              sub: "Calculado desde estados consolidados",
            ...
            },
            {
              ...
              sub: `de ${obligacionesAplicables.length} obligaciones`,
            ...
            },
            {
              label: "Con brechas",
              value: totalVencidas,
              sub: "Documental o estructural parcial",
        ...
        <Card className="border-none shadow-sm bg-white">
          <CardContent className="pt-5 pb-5">
            ...
            <div className="mt-3 flex items-center justify-between text-[11px] text-slate-400">
              <span className="flex items-center gap-1"><Cpu className="h-3 w-3" /> Datos obtenidos via Prisma</span>
              <span>{obligacionesAplicables.length} obligaciones aplicables</span>
            </div>
          </CardContent>
        </Card>

        <div>
          <h2 className="mb-4 text-sm font-semibold uppercase tracking-wide text-slate-500">Resultado por centro de trabajo</h2>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {resultadosPorEntidad.map((r) => {
              const centro = payload.centros.find((c) => c.id === r.entidadId);
              return <ResultadoCard key={r.entidadId} resultado={r} nombre={centro?.nombre ?? r.entidadId} />;
            })}
          </div>
        </div>

        <div>
          <h2 className="mb-4 text-sm font-semibold uppercase tracking-wide text-slate-500">Detalle de evaluaciones ({evaluaciones.length})</h2>
          <Card className="border-none shadow-sm bg-white">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {evaluaciones.map((ev, i) => {
                      const centro = payload.centros.find((c) => c.id === ev.entidadId);
                      const estadoCls =
                        ev.estado === "cumplido"
                      ...
                      return (
                        <tr key={`${ev.obligacionNombre}-${ev.entidadId}-${i}`} className="border-b last:border-0 hover:bg-slate-50/60 transition-colors">
                          <td className="py-2.5 pl-2 text-slate-800 font-medium max-w-xs">
                            <span className="line-clamp-1">{ev.obligacionNombre}</span>
                          </td>
                          <td className="py-2.5 text-slate-500 text-xs">{centro?.nombre ?? "Empresa"}</td>
                          <td className="py-2.5">
                            <span className={cn("inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-medium border", estadoCls)}>
                              ...
                              {ev.estado === "cumplido" ? "Cumplido" : ev.estado === "vencido" ? "Con brecha" : "Pendiente"}
                            </span>
                          </td>
                          <td className="py-2.5 text-xs text-slate-500">
                            <span className="inline-flex items-center gap-1 rounded-full px-2 py-0.5 bg-slate-100 text-slate-600 border border-slate-200">
                              <Cpu className="h-2.5 w-2.5" /> {fuenteLabel}
                            </span>
                          </td>
                          <td className="py-2.5 text-xs text-slate-500">
                            {ev.fuenteId ? (
                          ...
                          <td className="py-2.5 text-xs text-slate-500">{ev.fechaVencimiento ?? <span className="text-slate-300">-</span>}</td>
                        </tr>
                      );
                    })}
                  </tbody>
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-50 px-5 py-4">
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-400 mb-3">Proximos pasos</p>
          <div className="flex flex-wrap gap-2">
            {criticos > 0 && (
              ...
              >
                ...
                Revisar {criticos} hallazgo{criticos !== 1 ? "s" : ""} critico{criticos !== 1 ? "s" : ""}
            ...
            {totalPendientes > 0 && (
              ...
              >
                ...
                Revisar respaldo de {totalPendientes} obligacion{totalPendientes !== 1 ? "es" : ""}
                <ArrowRight className="h-3.5 w-3.5" />
              </Link>
            )}
          </div>
