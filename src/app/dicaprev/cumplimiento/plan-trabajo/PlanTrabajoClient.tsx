        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6 text-sm">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {(["pendiente", "en_proceso", "cerrada"] as EstadoAccionCumplimiento[])
...
export default function PlanTrabajoClient({
  ...
  const [filtroEstado, setFiltroEstado] = useState<EstadoAccionCumplimiento | "todos">("todos");
  const [filtroPrioridad, setFiltroPrioridad] = useState<PrioridadHallazgo | "todos">("todos");
  const [filtroCentro, setFiltroCentro] = useState("todos");

  // ── Handlers ──────────────────────────────────────────────────────────────

  const handleEstadoChange = (id: string, estado: EstadoAccionCumplimiento) => {
    setAcciones((prev) => prev.map((a) => (a.id === id ? { ...a, estado } : a)));
    setSelected((prev) => (prev?.id === id ? { ...prev, estado } : prev));
  };

  // ── KPIs ──────────────────────────────────────────────────────────────────

  const totalAuto = acciones.filter((a) => a.origenTipo === "automatico").length;
  const totalPendientes = acciones.filter((a) => a.estado === "pendiente").length;
  const totalEnProceso = acciones.filter((a) => a.estado === "en_proceso").length;
  const totalCerradas = acciones.filter((a) => a.estado === "cerrada").length;
      ...
      <div className="mx-auto max-w-7xl space-y-8 px-4 lg:px-0">
        ...
        <div className="flex items-center gap-2 rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800">
          ...
          <span>
            ...
            {totalAuto === 1 ? "acción generada" : "acciones generadas"} automáticamente ·{" "}
            {acciones.length - totalAuto} de hallazgos manuales.
          </span>
        </div>
        ...
        {(() => {
          ...
          return criticas.length > 0 ? (
            <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-800">
              ...
              <span>
                <strong>{criticas.length}</strong> acción{criticas.length !== 1 ? "es" : ""}{" "}
                crítica{criticas.length !== 1 ? "s" : ""} pendiente
                {criticas.length !== 1 ? "s" : ""} — requieren atención inmediata.
              </span>
              <button
                onClick={() => {
            ...
            <div className="flex flex-wrap gap-3 items-center">
              ...
              >
                ...
                <SelectContent>
                  ...
                  {(
                    Object.entries(ESTADO_CFG) as [EstadoAccionCumplimiento, { label: string }][]
                  ).map(([k, cfg]) => (
              ...
              <Select
                ...
                onValueChange={(v) => setFiltroPrioridad(v as PrioridadHallazgo | "todos")}
              >
                <SelectTrigger className="w-36 text-sm">
                ...
                <SelectContent>
                  ...
                  {(
                    Object.entries(PRIORIDAD_CFG) as [PrioridadHallazgo, { label: string }][]
                  ).map(([k, cfg]) => (
                    <SelectItem key={k} value={k}>
                      {cfg.label}
                    </SelectItem>
              ...
              <span className="ml-auto text-xs text-slate-400">
                {filtered.length} {filtered.length === 1 ? "acción" : "acciones"}
              </span>
            </div>
          </CardContent>
        </Card>
          ...
          {filtered.length === 0 ? (
            <CardContent className="py-20 text-center">
              ...
              <p className="text-xs text-slate-400 mt-1">
                Ajusta los filtros para ver otras acciones del plan de trabajo.
              </p>
            </CardContent>
          ) : (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1.5 text-xs text-slate-600">
                            ...
                            <span className="max-w-[130px] truncate">{accion.responsable}</span>
                          </div>
                        </td>
