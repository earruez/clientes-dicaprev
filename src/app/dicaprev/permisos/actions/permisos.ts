...
import type { PermisoInstalacion, PermisoOrganismo, PermisoResponsable } from "@prisma/client";
...
export async function crearPermiso(data: PermisoFormData) {
  ...
  if (data.fechaPresentacion) {
    fechaEstimadaResolucion = calcularFechaEstimadaResolucion(
      ...
      organismo.plazoDias,
      (organismo.tipoPlazo as string) || "NO_INFORMADO",
    );
  }
  ...
  try {
    const emailResult = await sendEmail({
      ...
      html: generarEmailPermiso(
        permiso as PermisoInstalacion & { organismo: PermisoOrganismo },
        responsable,
        "PERMISO_CREADO",
      ),
    });
...
export async function cambiarEstadoPermiso(
  ...
  try {
    const emailResult = await sendEmail({
      ...
      html: generarEmailPermiso(
        permisoActualizado as PermisoInstalacion & { organismo: PermisoOrganismo },
        permiso.responsable,
        "CAMBIO_ESTADO",
        {
          estadoAnterior,
