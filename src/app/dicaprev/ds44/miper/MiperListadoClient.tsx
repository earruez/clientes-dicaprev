  ...
  return <div className="space-y-5">
    ...
    <Card className="rounded-2xl border-slate-200 bg-white shadow-sm"><CardContent className="space-y-5 p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Button className="rounded-2xl font-semibold" onClick={() => setOpen((value) => !value)}>Nueva matriz MIPER</Button>
      </div>
      ...
      {open && <div className="grid gap-4 rounded-2xl border border-slate-200 bg-slate-50/60 p-5 md:grid-cols-2">
        ...
        <div className="flex gap-2 md:col-span-2"><Button className="rounded-2xl font-semibold" disabled={isPending} onClick={crear}>Crear matriz</Button><Button className="rounded-2xl" variant="outline" disabled={isPending} onClick={() => setOpen(false)}>Cancelar</Button></div>
      </div>}
