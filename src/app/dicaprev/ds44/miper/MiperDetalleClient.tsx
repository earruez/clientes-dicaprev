...
import { actualizarCabeceraDs44Miper, cambiarEstadoDs44Miper, guardarDs44MiperControl, guardarDs44MiperItem } from "./actions";
import type { MiperControl, MiperControlEstado, MiperControlTipo, MiperDetalleData, MiperEstado, MiperItem } from "./types";
...
const EMPTY_ITEM: ItemForm = { centroTrabajoId: "", areaId: "", cargoId: "", actividad: "", peligro: "", riesgo: "", consecuencia: "", probabilidad: 1, severidad: 1, responsableTrabajadorId: "", observaciones: "" };
...
export default function MiperDetalleClient({ data }: { data: MiperDetalleData }) {
  ...
  const riesgosCriticos = data.items.filter((item) => item.clasificacionRiesgo === "critico").length;
  ...
  function editarItem(item: MiperItem) {
    setItemForm({ itemId: item.id, centroTrabajoId: item.centroTrabajoId ?? "", areaId: item.areaId ?? "", cargoId: item.cargoId ?? "", actividad: item.actividad, peligro: item.peligro, riesgo: item.riesgo, consecuencia: item.consecuencia, probabilidad: item.probabilidad, severidad: item.severidad, responsableTrabajadorId: item.responsableTrabajadorId ?? "", observaciones: item.observaciones ?? "" });
  ...
  return <div className="space-y-5">
    ...
    <Card className="rounded-2xl border-slate-200 bg-white shadow-sm"><CardContent className="space-y-5 p-5">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex flex-wrap items-center gap-2"><h2 className="text-xl font-bold text-slate-900">{data.miper.codigo} · {data.miper.nombre}</h2><Badge variant="outline" className={ESTADOS[data.miper.estado].className}>{ESTADOS[data.miper.estado].label}</Badge></div><p className="mt-2 text-sm text-slate-500">Versión {data.miper.version} · Vigente desde {fecha(data.miper.vigenteDesde)} · Próxima revisión {fecha(data.miper.fechaProximaRevision)}</p></div><div className="flex flex-wrap gap-2">{data.editable && <Button variant="outline" className="rounded-2xl" onClick={() => setEditHeader((value) => !value)}>Editar cabecera</Button>}{data.editable && data.miper.estado === "borrador" && <Button variant="outline" className="rounded-2xl" disabled={isPending} onClick={() => run(() => cambiarEstadoDs44Miper({ miperId: data.miper.id, estado: "en_revision" }), "Matriz enviada a revisión.")}>Enviar a revisión</Button>}{data.editable && data.puedeAprobar && <Button className="rounded-2xl" disabled={isPending} onClick={() => run(() => cambiarEstadoDs44Miper({ miperId: data.miper.id, estado: "vigente" }), "Matriz declarada vigente.")}>Declarar vigente</Button>}{data.miper.estado !== "archivado" && <Button variant="outline" className="rounded-2xl text-rose-700" disabled={isPending} onClick={() => run(() => cambiarEstadoDs44Miper({ miperId: data.miper.id, estado: "archivado" }), "Matriz archivada.")}>Archivar</Button>}{data.miper.estado === "vigente" && <Button variant="outline" className="rounded-2xl" disabled>Nueva revisión próximamente</Button>}</div></div>
      {data.miper.observaciones && !editHeader && <p className="rounded-xl bg-slate-50 p-3 text-sm text-slate-700">{data.miper.observaciones}</p>}
      {editHeader && <div className="grid gap-4 rounded-2xl border border-slate-200 bg-slate-50/60 p-5 md:grid-cols-2"><label className="grid gap-1 text-sm font-medium">Nombre<input className={inputClass} value={nombre} onChange={(event) => setNombre(event.target.value)} /></label><label className="grid gap-1 text-sm font-medium">Próxima revisión<input type="date" className={inputClass} value={fechaRevision} onChange={(event) => setFechaRevision(event.target.value)} /></label><label className="grid gap-1 text-sm font-medium md:col-span-2">Observaciones<textarea className={`${inputClass} min-h-24`} value={observaciones} onChange={(event) => setObservaciones(event.target.value)} /></label><div className="flex gap-2 md:col-span-2"><Button className="rounded-2xl" disabled={isPending} onClick={() => run(() => actualizarCabeceraDs44Miper({ miperId: data.miper.id, nombre, fechaProximaRevision: fechaRevision, observaciones }), "Cabecera actualizada.", () => setEditHeader(false))}>Guardar</Button><Button variant="outline" className="rounded-2xl" onClick={() => setEditHeader(false)}>Cancelar</Button></div></div>}
    ...
    {showItemForm && <Card className="rounded-2xl border-slate-200 bg-white shadow-sm"><CardHeader><h2 className="text-lg font-bold text-slate-900">{itemForm.itemId ? "Editar ítem MIPER" : "Nuevo ítem MIPER"}</h2></CardHeader><CardContent className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      ...
      <label className="grid gap-1 text-sm font-medium">Probabilidad (1–5)<select className={inputClass} value={itemForm.probabilidad} onChange={(event) => setItemForm((value) => ({ ...value, probabilidad: Number(event.target.value) }))}>{[1,2,3,4,5].map((value) => <option key={value}>{value}</option>)}</select></label>
      <label className="grid gap-1 text-sm font-medium">Severidad (1–5)<select className={inputClass} value={itemForm.severidad} onChange={(event) => setItemForm((value) => ({ ...value, severidad: Number(event.target.value) }))}>{[1,2,3,4,5].map((value) => <option key={value}>{value}</option>)}</select></label>
      <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm"><p className="font-medium">Nivel calculado en servidor</p><p className="mt-1 text-slate-500">Vista referencial: {itemForm.probabilidad * itemForm.severidad}</p></div>
