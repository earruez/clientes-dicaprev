  ...
  return <div className="space-y-6">
    ...
    {plantillaData && <section className="grid gap-4 lg:grid-cols-2"><Card className="border-slate-200 shadow-sm"><CardHeader><h2 className="text-lg font-semibold text-slate-900">Completar {plantillaData.nombre}</h2></CardHeader><CardContent className="space-y-4">{plantillaData.campos.map((campo) => {
        ...
        return <label key={campo.key} className="block space-y-1 text-sm"><span className="flex items-center gap-2 font-medium text-slate-700">{campo.label}{campo.required ? " *" : ""}{responsableSeleccionado && esRecomendadoParaCampo(responsableSeleccionado, campo.filtroSugerido) && <Badge variant="outline" className="border-emerald-200 bg-emerald-50 text-emerald-700">Recomendado</Badge>}</span>
          {campo.type === "responsable" ? <><select className="w-full rounded-md border border-slate-300 p-2" value={responsableIds[campo.key] ?? ""} onChange={(event) => seleccionarResponsable(campo.key, event.target.value)}><option value="">Selecciona un trabajador responsable</option>{data.responsablesDisponibles.map((responsable) => <option key={responsable.trabajadorId} value={responsable.trabajadorId}>{responsable.nombre} — {responsable.cargoNombre}{esRecomendadoParaCampo(responsable, campo.filtroSugerido) ? " (Recomendado)" : ""}</option>)}</select>{responsableSeleccionado && <span className="block rounded-md bg-slate-50 px-3 py-2 text-xs text-slate-600"><strong className="text-slate-700">{responsableSeleccionado.nombre}</strong><br />{responsableSeleccionado.cargoNombre}{responsableSeleccionado.areaNombre ? ` · ${responsableSeleccionado.areaNombre}` : ""}</span>}</> : campo.type === "textarea" ? <textarea className="min-h-24 w-full rounded-md border border-slate-300 p-2" placeholder={campo.placeholder} value={campos[campo.key] ?? ""} onChange={(event) => setCampos((current) => ({ ...current, [campo.key]: event.target.value }))} /> : <input type={campo.type} className="w-full rounded-md border border-slate-300 p-2" placeholder={campo.placeholder} value={campos[campo.key] ?? ""} onChange={(event) => setCampos((current) => ({ ...current, [campo.key]: event.target.value }))} />}
        </label>;
      })}
      ...
      <Button disabled={isPending || data.responsablesDisponibles.length === 0 || faltanResponsables} onClick={submit}>Registrar documento</Button></CardContent></Card>
