...
const CRITICIDAD_CLS: Record<string, string> = {
  Alta: "bg-rose-100 text-rose-700 border border-rose-300",
  Media: "bg-amber-100 text-amber-700 border border-amber-300",
  Baja: "bg-sky-100 text-sky-700 border border-sky-300",
};

export default function DS44Tab() {
  ...
  return (
    <div className="space-y-8">
      ...
      <header className="flex items-start gap-3">
        ...
        <div>
          <h2 className="text-lg font-bold text-slate-900">Cumplimiento global DS44</h2>
          <p className="mt-0.5 text-sm text-slate-500">
            Visión ejecutiva del avance de implementación DS44 en todos los centros de trabajo.
          </p>
        </div>
      </header>

      {/* BLOQUE PRINCIPAL PREMIUM */}
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1.1fr_1fr] items-start">
          ...
          <div className="flex flex-col gap-6">
            <div className="flex items-center gap-6">
              <div className="relative h-32 w-32 flex-shrink-0 rounded-full border border-emerald-400/80 bg-slate-950 flex flex-col items-center justify-center shadow-[0_0_40px_rgba(16,185,129,0.35)]">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between rounded-2xl border border-slate-700 bg-slate-900/70 px-4 py-2">
                ...
                <span className="text-sm font-medium text-emerald-300">2 centros</span>
              </div>
              <div className="flex items-center justify-between rounded-2xl border border-slate-700 bg-slate-900/70 px-4 py-2">
                <div className="flex items-center gap-2 text-sm text-slate-100">
                ...
                <span className="text-sm font-medium text-amber-300">2 centros</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-emerald-400/60 bg-emerald-500/10 px-5 py-4">
              ...
              <p className="mt-2 text-sm text-emerald-50">
                Cerrar hallazgos críticos y actualizar evidencias en centros con cumplimiento inferior al{" "}
                ...
                <div className="mt-1 flex items-center justify-between text-[11px]">
                  ...
                  <span
                    className={
                      ...
                      (CRITICIDAD_CLS[item.criticidad] ?? "bg-slate-100 text-slate-600")
                    }
                  >
                    {item.criticidad}
                  </span>
          ...
          <CardContent className="space-y-3">
            {centrosTrabajo.map((centro, idx) => (
              <div
                ...
                className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5"
              >
                <p className="text-xs font-medium text-slate-700">{centro.nombre}</p>
                <div className="mt-2 flex items-center justify-between gap-2">
                  <div className="flex-1">
                    <Progress value={centro.cumplimiento} className="h-1.5" />
                    <p className="mt-1 text-[11px] text-slate-500">
                      Cumplimiento{" "}
                      <span className="text-slate-700">{centro.cumplimiento}%</span>
                    </p>
                  </div>
        ...
        <Card className="border border-slate-200 bg-white">
          ...
          <CardContent className="space-y-4">
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[11px] text-slate-500">
                    Avance: <span className="text-slate-700">{item.avance}%</span>
                  </span>
                </div>
                <p className="mt-2 text-xs font-medium text-slate-700">{item.titulo}</p>
            ...
            <div className="mt-1 flex items-center gap-2 text-[11px] text-slate-500">
              ...
              <span>Vista agregada · DS44 integrado en el sistema.</span>
            </div>
          </CardContent>
        </Card>
      </div>
